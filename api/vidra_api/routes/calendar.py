...
async def generate_calendar(
    ...
    month = CalendarMonth(persona_id=persona.id, month=payload.month, year=payload.year, mode="offline")
    db.add(month)
    await db.flush()
...
async def get_calendar(
    ...
    q = await db.execute(
        ...
        .options(selectinload(CalendarMonth.days).selectinload(CalendarDay.posts))
        .where(CalendarMonth.persona_id == persona_id, CalendarMonth.month == month, CalendarMonth.year == year, Persona.user_id == user.id)
    )
